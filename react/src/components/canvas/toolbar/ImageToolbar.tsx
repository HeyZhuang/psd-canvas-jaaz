...
export function ImageToolbar({ selectedElement }: ImageToolbarProps) {
  ...
  const handleCrop = () => {
    ...
    if (cropMode) {
      ...
      // 进入裁剪模式
      setCropMode(true)
      const width = Math.abs(selectedElement.width)
      const height = Math.abs(selectedElement.height)
      // 初始化裁剪区域为图片的完整大小
      setCropRect({
        x: 0,
        y: 0,
  ...
  const applyCrop = useCallback(async () => {
    ...
    try {
      ...
      if (!currentElement.fileId) return

      // 获取原始图片
      const files = excalidrawAPI.getFiles()
      const file = files[currentElement.fileId]
      ...
      const elementWidth = Math.abs(currentElement.width)
      const elementHeight = Math.abs(currentElement.height)
      ...
      // 更新文件
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      excalidrawAPI.addFiles(Object.values(newFiles) as any)
  ...
  }, [excalidrawAPI, selectedElement, cropRect])
